...
void run_simulator(FILE *arqE, char* type, int size_page, int size_memory)
{
    ...
    int index_Tpage, 
        n_pages,in_memory,offset = (int)(ceil(log2(size_page * 1024))), // 1 MB = 1024KB 
        time_zeroBits = 0, n_pos = 0, n_missingPages = 0, n_writtenPages = 0;
    unsigned int addr;
	char rw;
    int* pages;
    Frame* tablePages;
    ...
    while(fscanf(arqE, "%x %c", &addr, &rw) != EOF) 
    {
        ...
        //Caso não tenha sido carregado na memória ainda
